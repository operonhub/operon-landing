...
const RADAR_DOCUMENTATION_FILES = new Set(["README.md", "EDITORIAL_AUTOMATION.md"]);
 ...
 * @property {Array<{title: string, url: string, publisher?: string, kind?: "primary"|"institutional"|"secondary"}>} sources
 * @property {Array<{claim: string, sourceUrl: string}>=} claimSources
 * @property {{title?: string, description?: string, image?: string, imageFallback?: "operon-operational-plate-v1"}} seo
...
export function getAllArticles() {
  ...
  return fs
    ...
    .filter((fileName) => fileName.endsWith(".md") && !RADAR_DOCUMENTATION_FILES.has(fileName))
    .map(readArticleFile);
}
